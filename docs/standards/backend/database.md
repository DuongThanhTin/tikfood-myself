...
- Internal primary keys must be generated by the database. Do not use manually configured IDs such as `venue_001` as primary keys.
- User-facing or source-specific identifiers belong in separate fields such as `public_id`, `source`, and `source_external_id`.
- City, district, ward, and area filters must use canonical location IDs plus aliases. Raw text is display/import data, not the source of truth.
...
- address text deprecated compatibility field
- city text deprecated compatibility field
- district text deprecated compatibility field
- ward text deprecated compatibility field
- area_name text
- city_location_id uuid references locations(id)
- district_location_id uuid references locations(id)
- ward_location_id uuid references locations(id)
- area_location_id uuid references locations(id)
...
- `id`: internal database ID generated by PostgreSQL. Never rely on human-managed strings for primary keys.
- `public_id`: optional stable public identifier if the product needs a non-UUID ID in URLs, logs, or external contracts.
...
- `address_raw`: original imported address text from a user, partner, Google Places, social source, or admin tool.
- `address_display`: cleaned address string for UI display.
- `city_location_id`, `district_location_id`, `ward_location_id`, `area_location_id`: canonical location references used for filtering and ranking.
...
- `source_external_id`: source-owned ID such as a Google Place ID, TikTok place identifier, Instagram location ID, or admin import ID.

Raw `address`, `city`, `district`, and `ward` columns are compatibility fields only. New code should write `address_raw`, `address_display`, and canonical location IDs.

Indexes:

```sql
create index idx_venues_location on venues using gist (location);
create index idx_venues_city_location on venues (city_location_id);
create index idx_venues_district_location on venues (district_location_id);
create index idx_venues_slug on venues (slug);
create index idx_venues_public_id on venues (public_id);
create index idx_venues_source_external_id on venues (source, source_external_id);
...
Canonical administrative and product areas.

```sql
locations
- id uuid primary key default gen_random_uuid()
- type text not null -- country, city, district, ward, area
...
- Use `locations.id` for filtering and joins.
- Use `official_name` for display when no context-specific label is needed.
- Use `slug` for stable URL/filter values, for example `ho-chi-minh`, `quan-1`, `thao-dien`.
- `area` is for product/search neighborhoods that may not map one-to-one with official administrative units.

Indexes:

```sql
create index idx_locations_type_slug on locations (type, slug);
create index idx_locations_parent_type on locations (parent_id, type);
```

## location_aliases

Accepted spelling, abbreviation, language, and input variants for a canonical location.
...
- Lowercase input.
- Trim whitespace.
- Remove Vietnamese diacritics with PostgreSQL `unaccent`.
- Collapse repeated whitespace in application code when possible.
- Match exact `normalized_alias` first.
- Use trigram similarity only as fallback for typo-tolerance.

Indexes:

```sql
create index idx_location_aliases_normalized on location_aliases (normalized_alias);
create index idx_location_aliases_trgm on location_aliases using gin (normalized_alias gin_trgm_ops);
```
...
- district_location_id uuid references locations(id)
- area_location_id uuid references locations(id)
- area_name text compatibility/display field
- dish_id uuid references dishes(id)
...
Use location IDs for collection scoping. `area_name` is display/import compatibility only.
